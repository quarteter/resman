    ...
    <script>
        ...
        function bindSubmit() {
            $("#btnSubmit").on("click", function () {
                if ($("#noticeForm").valid()) {
                    $.post('/res/notice/save',$('#noticeForm').serializeJson(), function(data){
                        if(data.success){
                            tipSuccess("操作成功");
                            window.location.href="/res/notice/list";
                        }else{
                            tipError("操作失败");
                        }
                    });
...
<aside class="right-side">
    ...
    <section class="content">
        <form class="form-horizontal" role="form" method="post" id="noticeForm">
            <input type="hidden" name="id" value="${notice.id}"/>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="title">标题</label>

                <div class="col-sm-4">
                    <input id="title" class="form-control" name="title" value="${notice.title}"/>
                </div>
