            ...
            $("#btnSubmit").on("click", function () {
                if ($("#jobForm").valid()) {
                    $.post('/res/course/save', $('#jobForm').serializeJson(), function (data) {
                        if (data.success) {
                            tipSuccess("操作成功");
                            window.location.href = "/res/course/list?pid=${job.parent}";
                        } else {
                            tipError("操作失败");
                        }
                    });
...
<aside class="right-side">
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">修改作业</li>
        </ol>
    </section>
    <section class="content">
        <form class="form-horizontal" role="form" method="post" id="jobForm">
            <input type="hidden" name="id" value="${job.id}"/>
            <input type="hidden" name="parent" value="${job.parent}"/>
            <input type="hidden" name="ntype" value="1"/>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="name">名称</label>
                <div class="col-sm-4">
                    <input id="name" class="form-control" name="name" value="${job.name}"/>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="description">备注</label>
                <div class="col-sm-4">
                    <input id="description" class="form-control" name="description" value="${job.description}"/>
                </div>
            </div>
